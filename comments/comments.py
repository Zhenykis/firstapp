...
from repository.comments import create_comment as create_com, del_comment
router = APIRouter(
    prefix="/comments",
    tags=["comment"],
)
...
async def  delete_comment(comment_id: int, db:AsyncSession = Depends(get_db)):
    try:
        await del_comment(comment_id=comment_id, db=db)
    except CommentNotFound as e:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Комментарий отсутствует!"
        )
